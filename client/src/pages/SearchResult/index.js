...
class SearchResult extends Component {
            ...
            API.getListingById(item.listing).then((listing) => {
              ...
              API.getListingByIdAndProximity(longLatArray).then((item) => {
                for (let i = 0; i < item.data.length; i++) {
                  if (listing.data[0].id === item.data[i].id) {
                    this.setState({
  ...
  renderMap = () => {
    ...
    loadScript(
      "http://maps.googleapis.com/maps/api/js?key=AIzaSyAqMhysRXqdWYWpzfxHxkxe3_SqVP-UnIo&callback=initMap"
    );
